 ...
 */
gpu::NeuralNetwork::NeuralNetwork(int hidden_layer1_size,
 ...
 */
void gpu::NeuralNetwork::fit(gpu::Dataset& X_train_stand, std::vector<float>& y_train){
 ...
 * 
 */
float gpu::NeuralNetwork::forwardPropegation(){
...
void gpu::NeuralNetwork::x(const gpu::Vector& x){
    this->m_x = x;
}

void gpu::NeuralNetwork::y(const float& y){
    this->m_y = y;
}